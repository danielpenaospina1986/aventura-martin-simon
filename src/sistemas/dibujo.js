...
export function generarTexturas(escena) {
  ...
  // caucho flotando dentro. Provisional hasta que llegue su dibujo, pero con
  // los pinceles de tinta de siempre para que hable el mismo idioma.
  const astronauta = (modo) => (g) => {
    const a = JEFE.ancho;
    const h = JEFE.alto;
    const cx = a / 2;

    // botas, bien gordas; cuando esta atascado van llenas de espuma
    const anchoBota = 44;
    tintaRedonda(g, cx - 58, h - 34, anchoBota, 32, 12, COLORES.trajeSombra, 5);
    tintaRedonda(g, cx + 14, h - 34, anchoBota, 32, 12, COLORES.trajeSombra, 5);

    // cuerpo del traje
    tintaRedonda(g, cx - 46, h - 108, 92, 82, 26, COLORES.trajeClaro, 6);
    // cinturon y mochila
    g.fillStyle(COLORES.trajeSombra, 1);
    g.fillRect(cx - 44, h - 62, 88, 12);
    tintaRedonda(g, cx + 40, h - 100, 22, 46, 8, COLORES.trajeSombra, 5);

    // brazos
    tintaRedonda(g, cx - 66, h - 100, 22, 52, 10, COLORES.trajeClaro, 5);
    tintaRedonda(g, cx + 44, h - 100, 22, 52, 10, COLORES.trajeClaro, 5);

    // el casco: una pecera con agua jabonosa
    const casco = 46;
    const cascoY = h - 132;
    tintaCirculo(g, cx, cascoY, casco, COLORES.trajeClaro, 6);
    // el agua, que llena la mitad de abajo
    g.fillStyle(COLORES.aguaJabon, 0.95);
    g.slice(cx, cascoY, casco - 5, 0, Math.PI, false);
    g.fillPath();
    // espuma en la linea del agua
    g.fillStyle(COLORES.espuma, 1);
    [[-30, 2, 9], [-12, -2, 11], [8, 1, 10], [27, 3, 8]].forEach(([dx, dy, r]) =>
      g.fillCircle(cx + dx, cascoY + dy, r),
    );

    // el patico de caucho, flotando
    const patoY = cascoY + 12;
    tintaCirculo(g, cx + 10, patoY, 13, COLORES.patico, 4);
    tintaCirculo(g, cx + 24, patoY - 10, 9, COLORES.patico, 4);
    g.fillStyle(0xe8853a, 1);
    g.fillTriangle(cx + 31, patoY - 12, cx + 42, patoY - 9, cx + 31, patoY - 5);
    g.fillStyle(TINTA, 1);
    g.fillCircle(cx + 27, patoY - 12, 2.2);

    // la cara, detras del agua
    const ojoY = cascoY - 14;
    if (modo === 'mareado') {
      // aspas en vez de ojos y la lengua fuera
      g.lineStyle(5, TINTA, 1);
      [[-16, ojoY], [16, ojoY]].forEach(([dx, y]) => {
        g.beginPath();
        g.moveTo(cx + dx - 7, y - 7);
        g.lineTo(cx + dx + 7, y + 7);
        g.moveTo(cx + dx + 7, y - 7);
        g.lineTo(cx + dx - 7, y + 7);
        g.strokePath();
      });
    } else {
      tintaCirculo(g, cx - 16, ojoY, 12, 0xffffff, 4);
      tintaCirculo(g, cx + 16, ojoY, 12, 0xffffff, 4);
      g.fillStyle(TINTA, 1);
      const mira = modo === 'atascado' ? 0 : 4;
      g.fillCircle(cx - 16 + mira, ojoY + 2, 5.5);
      g.fillCircle(cx + 16 + mira, ojoY + 2, 5.5);
      // cejas: enfadado cuando pisa, resignado cuando se atasca
      g.lineStyle(5, TINTA, 1);
      g.beginPath();
      if (modo === 'atascado') {
        g.moveTo(cx - 26, ojoY - 16);
        g.lineTo(cx - 6, ojoY - 12);
        g.moveTo(cx + 26, ojoY - 16);
        g.lineTo(cx + 6, ojoY - 12);
      } else {
        g.moveTo(cx - 27, ojoY - 14);
        g.lineTo(cx - 7, ojoY - 6);
        g.moveTo(cx + 27, ojoY - 14);
        g.lineTo(cx + 7, ojoY - 6);
      }
      g.strokePath();
    ...
    brillo(g, cx - casco * 0.45, cascoY - casco * 0.45, 9);

    // atascado: espuma desbordando por las botas
    if (modo === 'atascado') {
      g.fillStyle(COLORES.espuma, 1);
      [
        [cx - 58, h - 30, 14], [cx - 40, h - 22, 11], [cx - 22, h - 28, 9],
        [cx + 16, h - 26, 12], [cx + 38, h - 22, 10], [cx + 54, h - 30, 13],
      ].forEach(([x, y, r]) => g.fillCircle(x, y, r));
      g.lineStyle(3, COLORES.aguaJabon, 0.9);
      g.strokeCircle(cx - 40, h - 22, 11);
      g.strokeCircle(cx + 38, h - 22, 10);
    }
  };
